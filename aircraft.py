        ...
        while True:
            ...
            if (len(received_data) > 3):
                if (received_data[0:3] == "MSG"):
                    adsb_messages.append(received_data.split(","))
    except:
...
def prune_messages(adsb_messages, file):
    ...
    utils.debug_message("Pruned " + str(messages_pruned_count) + " messages")
...
def message_file_maintainer():
    ...
    while True:
        adsb_messages =  prune_messages(adsb_messages, file)
...
def fetch_aircraft_data():
    ...
    for entry in adsb_messages: # Iterate through each entry in the CSV list data.
...
def adsb_alert_processing(current_location, current_speed):
    if (config["general"]["adsb_alerts"]["enabled"] == True and config["general"]["gps"]["enabled"] == True): # Check to see if ADS-B alerts are enabled.
        ...
        aircraft_data = fetch_aircraft_data() # Fetch the most recent aircraft data.

        aircraft_threats = [] # Set the list of active aircraft threats to an empty placeholder database.
        sorted_aircraft_threats = {} # This is a placeholder that will hold all aircraft threats sorted by threat level and distance (with the aircraft ID as the key)
        ...
        return sorted_aircraft_threats, aircraft_data # Return the processed information.
    else:
