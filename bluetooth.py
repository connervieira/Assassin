...
def process_bluetooth_alerts(devices, current_position):
    ...
    for device in devices:
        if (device in bluetooth_device_data): # Check to see if this device does not yet exist in the complete device data.
            if (time.time() - bluetooth_device_data[device]["seen"]["original"]["time"] > config["general"]["bluetooth_scanning"]["memory_time"] * 60): # Check to see if it has been at least a certain number of minutes since this device was seen.
